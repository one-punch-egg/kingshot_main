...
def get_code_data():
    try:
        ...
        # Matches the specific HTML structure
...
def run():
    ...
    active_codes_on_site = {
        item["code"]: item for item in current_data
    }

    session = requests.Session()

    # --- PART A: Handle New or Still Active Codes ---
    # Iterate reversed so new codes appear at bottom
    for code in reversed(list(active_codes_on_site.keys())):
        ...
        # Reactivate expired message if code returns
    ...
    # --- PART B: Handle Expired Codes ---
    for code, data in msg_map.items():

        if (
            code not in active_codes_on_site
